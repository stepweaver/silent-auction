...
export async function sendWinnerDigest({ email, bidderName, items, paymentInstructions, pickupInstructions, contactEmail, paymentInstructionsUrl }) {
    ...
    .map(
      (item) => `
        <tr style="border-bottom: 1px solid #e5e7eb;">
          ...
          <td style="padding: 12px; text-align: right; font-weight: bold; color: #2563eb;">
            $${Number(item.winningBid).toFixed(2)}
          </td>
          <td style="padding: 12px; text-align: right;">
            ${item.itemUrl ? `<a href="${item.itemUrl}" style="color: #2563eb; text-decoration: none; font-weight: 600;">View item →</a>` : ''}
          </td>
        </tr>
      `,
    )
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; color: #1f2937;">
      ${logoUrl ? `<div style="text-align:center;margin-bottom:12px;"><img src="${logoUrl}" alt="" style="max-height:48px"/></div>` : ''}
      <h2 style="color: #2563eb; margin-bottom: 8px;">Congratulations${bidderName ? `, ${bidderName}` : ''}! 🎉</h2>
      <p style="font-size: 15px; line-height: 1.5;">
        Thank you for supporting the Mary Frank Elementary PTO silent auction! You won the following item${safeItems.length > 1 ? 's' : ''}:
      </p>
      ...
      <div style="margin: 24px 0; padding: 20px; background: #ecfdf5; border-radius: 12px;">
        ...
        ${paymentInstructionsUrl ? `<p style="margin: 16px 0 0 0;"><a href="${paymentInstructionsUrl}" style="display: inline-block; padding: 12px 20px; background: #10b981; color: white; border-radius: 8px; text-decoration: none; font-weight: bold;">View Payment Instructions</a></p>` : ''}
      </div>
      <p style="font-size: 14px; color: #4b5563;">If you have any questions, ${contactEmail ? `please contact us at <a href="mailto:${contactEmail}" style="color: #2563eb;">${contactEmail}</a>.` : 'please reach out to the auction administrators.'}</p>
      ${emailFooter({ contactEmail })}
    </div>
  `;
...
export async function sendAliasAccessNotification({ email, alias, contactEmail, siteUrl }) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      ${siteUrlBase ? `<p><a href="${siteUrlBase}" style="display: inline-block; padding: 12px 24px; background: var(--primary-500); color: white; text-decoration: none; border-radius: 6px; margin-top: 16px;">View Auction</a></p>` : ''}
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
        ...
        <a href="${verificationLink}" style="display: inline-block; padding: 12px 24px; background: var(--primary-500); color: white; text-decoration: none; border-radius: 6px; font-weight: bold;">Verify Email Address</a>
      </div>
      <p style="font-size: 14px; color: #555;">Or copy and paste this link into your browser:</p>
      <p style="font-size: 12px; color: #666; word-break: break-all; background: #f9fafb; padding: 12px; border-radius: 4px; font-family: monospace;">${escapedLink}</p>
