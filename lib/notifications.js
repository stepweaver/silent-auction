...
// Resend rate limit: 2 requests/second. Throttle to stay under.
export const RESEND_THROTTLE_MS = 600;
...
export async function sendEmail(to, subject, html, { fromName, replyTo, text } = {}) {
  if (process.env.DEMO_MODE === 'true') {
    logInfo('Demo mode: email suppressed', { to: maskEmail(to), subject });
    return true;
  }
...
export async function sendAdminWinnersList({
  ...
  if (safeWinners.length === 0 && safeDonors.length === 0) {
    ...
    for (let i = 0; i < adminEmails.length; i++) {
      ...
      if (await sendEmail(adminEmails[i], 'Auction Closed', html, { fromName, replyTo, text })) sentCount++;
    }
    return sentCount;
  }
  ...
  for (let i = 0; i < adminEmails.length; i++) {
    if (i > 0) await delay(RESEND_THROTTLE_MS);
    if (await sendEmail(adminEmails[i], subject, html, { fromName, replyTo, text })) sentCount++;
  }
  return sentCount;
}
