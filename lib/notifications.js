...
export async function sendEmail(to, subject, html, { fromName, replyTo, text } = {}) {
  ...
  try {
    ...
    if (error) {
      console.error('Resend email error:', JSON.stringify(error), '| to:', to, '| subject:', subject);
      return false;
    }

    console.log('Email sent successfully via Resend:', data?.id, '| to:', to);
    return true;
  } catch (error) {
    console.error('Email notification error:', error, '| to:', to, '| subject:', subject);
    return false;
  }
}
