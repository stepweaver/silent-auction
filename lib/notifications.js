...
async function sendSMSNotification(to, itemTitle, currentBid, itemUrl) {
  ...
  try {
    let plivo;
    try {
      // Use a function to construct the module name at runtime
      // This prevents Next.js/webpack from statically analyzing the import
      const getModuleName = () => 'plivo';
      plivo = await import(getModuleName());
    } catch (importError) {
