 ...
 * CSRF is required for state-changing routes (bid, donate, alias create, etc.).
 * Client must send x-csrf-token header (get token from GET /api/csrf-token).
 *
 * @param {Request} request - Next.js request object
 * @param {string} sessionId - Optional session identifier
 * @returns {boolean} True if valid, false otherwise (missing or invalid token)
 */
