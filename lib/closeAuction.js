...
async function getAllWinners(supabase, items) {
  ...
  let winners = await Promise.all(winnerPromises);
  winners = winners.filter(Boolean);

  // Prefer user_aliases.name over bids.bidder_name (canonical source).
  // Bids may have stored alias when localStorage was stale or cleared.
  const emails = [...new Set(winners.map((w) => w.email?.trim().toLowerCase()).filter(Boolean))];
  if (emails.length > 0) {
    ...
    if (aliases) {
      for (const a of aliases) {
        ...
        if (key && a.name?.trim()) {
          nameByEmail.set(key, a.name.trim());
        }
      }
    }
    ...
    for (const w of winners) {
      ...
      if (key && nameByEmail.has(key)) {
        w.bidderName = nameByEmail.get(key);
      }
    }
  }
...
export async function sendClosingEmailsOnly({ triggeredBy = 'manual' } = {}) {
  ...
  const { delay, RESEND_THROTTLE_MS } = await import('@/lib/notifications');

  // Send admin emails first (fewer recipients) so admins get notified quickly
  let adminEmailsSent = 0;
  if (adminEmails.length > 0) {
    try {
      const { sendAdminWinnersList } = await import('./notifications.js');
      adminEmailsSent = await sendAdminWinnersList({
        adminEmails,
        winners,
        donors,
        contactEmail,
  ...
  // Send winner emails (throttled to stay under Resend 2 req/sec limit)
  const winnerEntries = Array.from(winnersByEmail.values());
  let emailsSent = 0;
  for (let i = 0; i < winnerEntries.length; i++) {
    if (i > 0) await delay(RESEND_THROTTLE_MS);
    const ok = await sendWinnerDigest({
      email: winnerEntries[i].email,
      bidderName: winnerEntries[i].bidderName,
      items: winnerEntries[i].items,
...
export async function closeAuction({ force = false, triggeredBy = 'manual' } = {}) {
  ...
  // Send admin emails first so admins get notified quickly
  let adminEmailsSent = 0;
  if (adminEmails.length > 0) {
    try {
      const { sendAdminWinnersList } = await import('./notifications.js');
