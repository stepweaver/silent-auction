...
export async function closeAuction({ force = false, triggeredBy = 'manual' } = {}) {
  ...
  // Fetch all donation pledges and send payment instructions
  let donationEmailsSent = 0;
  let donationsCount = 0;
  let donationsTotal = 0;
  try {
    const { data: allDonations, error: donationsError } = await supabase
      .from('donations')
      .select('id, donor_name, email, amount, message');
    ...
    if (donationsError) {
      ...
      donationsTotal = allDonations.reduce((sum, d) => sum + Number(d.amount || 0), 0);

      // If a dedicated online donations payment link is configured, donors
      // already receive payment instructions in their confirmation email.
      // In that case, skip sending separate donor payment digests at close.
      if (!process.env.CHEDDARUP_PAYMENT_URL_DONATIONS) {
        const { sendDonorPaymentDigest } = await import('@/lib/notifications');

        // Group donations by email
        const donationsByEmail = new Map();
        for (const donation of allDonations) {
          const emailKey = donation.email?.trim().toLowerCase();
          if (!emailKey) continue;

          const existing = donationsByEmail.get(emailKey) || {
            email: donation.email.trim(),
            donorName: donation.donor_name || '',
            donations: [],
          };

          if (!existing.donorName && donation.donor_name) {
            existing.donorName = donation.donor_name;
          }
          ...
          donationsByEmail.set(emailKey, existing);
        }

        const donorEmailPromises = Array.from(donationsByEmail.values()).map((entry) =>
          sendDonorPaymentDigest({
            email: entry.email,
        ...
        donationEmailsSent = donorEmailResults.filter((result) => result.status === 'fulfilled').length;
      }
    }
  } catch (donationError) {
    console.error('closeAuction:donation emails error', donationError);
  }

  // Send admin winners list email
  let adminEmailsSent = 0;
  if (adminEmails.length > 0 && (winners.length > 0 || donationsCount > 0)) {
    try {
